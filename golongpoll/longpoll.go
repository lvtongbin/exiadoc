	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case <-time.After(time.Duration(timeout) * time.Second):
			...
			if jsonData, err := json.Marshal(timeoutResp); err == nil {
				...
				io.WriteString(w, "{\"code\": 1063, \"errmsg\": \"json marshaller failed\"}")
			}
		case events := <-subscription.Events:
			...
			opEvents := make([]resEvent, 0, len(events))
			for _, v := range events {
				event := resEvent{v.Timestamp / 1000, v.Data}
				opEvents = append(opEvents, event)
			}
			if jsonData, err := json.Marshal(eventResponse{0, &opEvents}); err == nil {
				io.WriteString(w, string(jsonData))
			} else {
				io.WriteString(w, "{\"code\": 1063, \"errmsg\": \"json marshaller failed\"}")
			}
...
func makeTimeoutResponse(t time.Time) *timeoutResponse {
	return &timeoutResponse{0, "no events before timeout",
